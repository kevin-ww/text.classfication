...
public class RuleExecutionSetTest extends TestCase {
  /**
   * RuleExecutionSetTest constructor. This is the default constructor for a
   * JUnit TestCase.
   * 
   * @param name
  ...
  /**
   * Test the compliance for javax.rules.admin.RuleExecutionSet. Get a
   * RuleAdministrator from a RuleServiceProvider and get the
   * LocalRuleExecutionSetProvider from the Administrator. Create a
   * RuleExecutionSet via the LocalRuleExecutionSetProvider. An input stream to
   * the tck_res_1.xml rule execution set is used to construct the rule
   * execution set. Test whether or not properties can be set and retrieved,
   * object filters can be set and retrieved. Iterate through the rules of this
   * rule execution set.
   * 
   * <p>
   * <b>Description:</b><br>
   ...
   * <li>Create Instance (with and without properties)
   * <ul>
   * <li>Fail: If any errors occur during the creation of the rule execution
   * set.
   * <li>Succeed: If an instance of a RuleExecutionSet could successfully be
   * created and the specified properties are available.
   * </ul>
   * <li>Get/Set properties
   * <ul>
   * <li>Fail: If the properties could not be set or retrieved.
   * <li>Succeed: If all the properties can successfully be set and retrieved.
   * </ul>
   * <li>Get/Set object filters
   * <ul>
   * <li>Fail: If a failure occurs during the set or get of an ObjectFilter.
   * <li>Succeed: If an ObjectFilter can successfully be set and retrieved.
   * </ul>
   * <li>Retrieve Rules from the RuleExecutionSet
   * <ul>
  ...
  public void testRuleExecutionSet() {
    try {
      ...
      RuleServiceProvider serviceProvider = TestCaseUtil
          .getRuleServiceProvider("RuleExecutionSetTest");
      ...
      assertNotNull("[RuleExecutionSetTest] " + "RuleAdministrator not found.",
          ruleAdministrator);
      ...
      InputStream inStream = TestCaseUtil
          .getRuleExecutionSetInputStream("tck_res_1.xml");
      ...
      RuleExecutionSet res = localProvider.createRuleExecutionSet(inStream,
          creationMap);
      ...
      assertNotNull("[RuleExecutionSetTest] " + "No description found.",
          res.getDescription());
      assertNotNull("[RuleExecutionSetTest] " + "No name found.", res.getName());
      ...
      assertNotNull("[RuleExecutionSetTest] " + "Could not retrieve property",
          obj);
      ...
      assertEquals("[RuleExecutionSetTest] ", filter.getClass().getName(),
          objectFilter);

      // Test the getRules.
      List rules = res.getRules();
      assertNotNull("[RuleExecutionSetTest] " + "No rules found.", rules);
      assertTrue("[RuleExecutionSetTest] ", (0 < rules.size())); // At least one
                                                                 // rule.
    } catch (Exception e) {
      fail(e.getMessage());
    }
