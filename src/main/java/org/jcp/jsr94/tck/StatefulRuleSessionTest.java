 ...
 * <p>
 * The definition of this rule execution set can be found in the "tck_res_2.xml"
 * file.<br>
 ...
 * invocation will add additional information to the rule execution set and the
 * processing will involve both the newly provided information as well as the
 * processed information of the previous execution.
 * <p>
 * The rule execution set must have support for the following business object
 * model:
 * <ul>
 * <li>Customer Class.<br>
 * The Customer business object is a simple business object that contains a name
 * and credit limit property. The definition of this class can be found in
 * {@link org.jcp.jsr94.tck.model.Customer}
 * <li>Invoice Class.<br>
 * The Invoice business object is a simple business object that contains a
 * description, amount, and status property. The definition of this class can be
 * found in {@link org.jcp.jsr94.tck.model.Invoice}
 * </ul>
 ...
 * and the status of the invoice is unpaid then decrement the credit limit with
 * the amount of the invoice and set the status of the invoice to "paid".
 * </ul>
 * <p>
 * <b>Note:</b><br>
 * Additional physical rules may be defined to accomplish the requirements
 * mentioned above.
 * <p>
...
public class StatefulRuleSessionTest extends TestCase {
  /**
   * StatefulRuleSessionTest constructor. This is the default constructor for a
   * JUnit TestCase.
   * 
   * @param name
  ...
  /**
   * Test the compliance for javax.rules.StatefulRuleSession. Create a stateful
   * rule session, add a Customer and Invoice object and verify whether those
   * objects can be retrieved be their Handle. Remove the objects via the handle
   * and reset the session. Add the object (this time all at once via a List)
   * and execute the rules. Verify the results and add another Invoice object.
   * Verify the results again. This time also check whether the state in the
   * Customer object was correctly maintained. Reset the session and add the
   * objects one more time. Execute the rules and this time apply an object
   * filter to the result set.
   * <p>
   * <b>Description:</b><br>
   * <ul>
   * <li>Create Stateful rule session
   * <ul>
   * <li>Fail: If any error occurs while creating the stateful rule session.
   * <li>Succeed: If a stateful rule session can succesfully be created.
   * </ul>
   * <li>Add and Remove and Get objects
   * <ul>
   * <li>Fail: If any of the add, get, and remove operations do not return the
   * expected results.
   * <li>Succeed: If add, get and remove are succesfully handling the rule
   * session interaction.
   * </ul>
   * <li>Execute rules
   * <ul>
   * <li>Fail: If any error occurs while executing the rules.
   * <li>Succeed: If the execute rules is successful and the result set is
   * according to the specifications.
   * </ul>
   * <li>Get processed objects
   * <ul>
   * <li>Fail: If the result set is not in compliance with the specifications.
   * <li>Succeed: If all the object can successfully be retrieved.
   * </ul>
   * <li>Object filtering
   * <ul>
   * <li>Fail: If not all objects in the result set are filtered correctly.
   * <li>Succeed: If only the by the object filter specified objects can be
  ...
  public void testStatefulRuleSession() {
    try {
      StatefulRuleSession ruleSession = TestCaseUtil.getStatefulRuleSession(
          "stateful", "tck_res_2.xml");
      ...
      assertTrue("[StatefulRuleSessionTest] ", (obj instanceof Customer));
      ...
      assertEquals("[StatefulRuleSessionTest] ", 2, list.size());
      ...
      assertEquals("[StatefulRuleSessionTest] verify updateObject ", 8000,
          updateCustomer.getCreditLimit());
      ...
      while (itr.hasNext()) {
        ...
        obj = ruleSession.getObject((Handle) obj); // Get the object from the
                                                   // handle.
        if (obj instanceof Customer)
          resultCustomer = (Customer) obj;
        if (obj instanceof Invoice)
      ...
      assertNotNull("[StatefulRuleSessionTest] ", list);
      assertEquals("[StatefulRuleSessionTest] ", 1, list.size());
      ...
      assertNull(
          "[StatefulRuleSessionTest] " + "Customer found in result set.",
          resultCustomer);
      assertNotNull("[StatefulRuleSessionTest] "
          + "Invoice not found in result set.", resultInvoice);
      ...
      assertNotNull("[StatefulRuleSessionTest] " + "No results found.", results);
      assertEquals("[StatefulRuleSessionTest] ", 2, results.size());
      ...
      assertEquals("[StatefulRuleSessionTest] verify Customer ", 3000,
          resultCustomer.getCreditLimit());
      assertEquals("[StatefulRuleSessionTest] verify Invoice ", "paid",
          resultInvoice.getStatus());
      ...
      ObjectFilter customerFilter = new TestObjectFilter(
          TestObjectFilter.CUSTOMER_FILTER);
      ObjectFilter invoiceFilter = new TestObjectFilter(
          TestObjectFilter.INVOICE_FILTER);

      // Get the customer
      results = ruleSession.getObjects(customerFilter);
      assertNotNull("[StatefulRuleSessionTest] " + "No results found.", results);
      assertEquals("[StatefulRuleSessionTest] ", 1, results.size());
      ...
      assertNotNull("[StatefulRuleSessionTest] "
          + "Customer not found in result set.", resultCustomer);
      assertNull("[StatefulRuleSessionTest] " + "Invoice found in result set.",
          resultInvoice);
      ...
      // to 1500.
      assertEquals("[StatefulRuleSessionTest] verify Customer(2) ", 1500,
          resultCustomer.getCreditLimit());
