 ...
 * <p>
 * The definition of the rule execution set can be found in the "tck_res_1.xml"
 * file.<br>
 * This RuleExecutionSet will be invoked by the TCK in a stateless manner.<br>
...
public class StatelessRuleSessionTest extends TestCase {
  /**
   * StatelessRuleSessionTest constructor. This is the default constructor for a
   * JUnit TestCase.
   * 
   * @param name
  ...
  /**
   * Test the compliance for javax.rules.StatelessRuleSession. Create a
   * stateless rule session and execute the rules on a Customer and Invoice
   * object. The execution of the rule will be invoked with an without an object
   * filter. The object filter is the TestObjectFilter of the TCK.
   * 
   * <p>
   * <b>Description:</b><br>
   * <ul>
   * <li>Create stateless rule session
   * <ul>
   * <li>Fail: If any exception occurs while creating the stateless rule session
   * <li>Succeed: If the stateless rule session can successfully be created.
   * </ul>
   * <li>Execute Rules and verify results
   * <ul>
   * <li>Fail: If any error occurs during execution of the rules or if the
   * results do not match the expected output.
   * <li>Succeed: If the execute rules produced the expected output.
   * </ul>
   * <li>Execute Rules with object filtering
   * <ul>
   * <li>Fail: If any error occurs during the execution of the rules or if the
   * results to not match the expected output.
   * <li>Succeed: If the execution of the rules produced the correct set of
   * output objects after filtering.
   * </ul>
   * </ul>
   * 
  ...
  public void testStatelessRuleSession() {
    try {
      StatelessRuleSession ruleSession = TestCaseUtil.getStatelessRuleSession(
          "stateless", "tck_res_1.xml");
      ...
      // Check the results.
      assertNotNull("[StatelessRuleSessionTest] " + "No results found.",
          results);
      ...
      assertEquals("[StatelessRuleSessionTest] ", 3000,
          resultCustomer.getCreditLimit());
      assertEquals("[StatelessRuleSessionTest] ", "paid",
      ...
      assertEquals("[StatelessRuleSessionTest] ", 1, results.size());
      ...
      assertNotNull("[StatelessRuleSessionTest] "
          + "Customer not found in result set.", resultCustomer);
      assertNull(
          "[StatelessRuleSessionTest] " + "Invoice found in result set.",
          resultInvoice);
