...
public class RuleRuntimeTest extends TestCase {
  /**
   * RuleRuntimeTest constructor. This is the default constructor for a JUnit
   * TestCase.
   * 
   * @param name
  ...
  /**
   * Test the compliance for javax.rules.RuleRuntime. This test will get the
   * rule service provider and the corresponding rule administrator. Both the
   * stateless (tck_res_1.xml) and stateful (tck_res_2.xml) rule execution sets
   * will be registered. The RuleRuntime is requested from the rule service
   * provider and the RuleRuntime interface API definition is tested.
   * <p>
   * <b>Description:</b><br>
   * <ul>
   * <li>Retrieve RuleRuntime
   * <ul>
   * <li>Fail: If no RuleRuntime instance is returned by the rule service
   * provider.
   * <li>Succeed: If a RuleRuntime instance is returned by the rule service
   * provider.
   * </ul>
   * <li>Registration
   * <ul>
   * <li>Fail: If the registered stateless and stateful rule execution sets are
   * not part of the retrieved registration list.
   * <li>Succeed: If the registered rule execution sets are retrieved.
   * </ul>
   * <li>Create Stateless Rule session
   * <ul>
   * <li>Fail: If any error occurs when creating a stateless rule session.
   * <li>Succeed: If a rule session can successfully be created and is an
   * instance of StatelessRuleSession.
   * </ul>
   * <li>Create Stateful Rule session
   * <ul>
   * <li>Fail: If any error occurs when creating a stateful rule session.
   * <li>Succeed: If a rule session can successfully be created and is an
   * instance of StatefulRuleSession.
   * </ul>
   * </ul>
   * 
  ...
  public void testRuleRuntime() {
    try {
      ...
      RuleServiceProvider serviceProvider = TestCaseUtil
          .getRuleServiceProvider("ruleRuntimeTest");
      assertNotNull("[RuleRuntimeTest] " + "RuleServiceProvider not found.",
          serviceProvider);

      // Get the RuleAdministrator
      RuleAdministrator ruleAdministrator = serviceProvider
          .getRuleAdministrator();
      assertNotNull("[RuleRuntimeTest] " + "RuleAdministrator not found.",
          ruleAdministrator);

      // Get an input stream for the stateless test XML rule
      // execution set.
      // Try to load the files from the "rule-execution-set-location".
      InputStream inStream = new FileInputStream(location + "/" + statelessUri);
      assertNotNull("[RuleRuntimeTest] " + "Input stream for " + location + "/"
      ...
      RuleExecutionSet res = ruleAdministrator
          .getLocalRuleExecutionSetProvider(null).createRuleExecutionSet(
              inStream, null);
      assertNotNull("[RuleRuntimeTest] " + "RuleExecutionSet " + statelessUri
          + " could not be created.", res);
      inStream.close();

      // register the RuleExecutionSet
      ruleAdministrator.registerRuleExecutionSet(statelessUri, res, null);

      // Get an input stream for the stateful test XML rule
      // execution set.
      // Try to load the files from the "rule-execution-set-location".
      inStream = new FileInputStream(location + "/" + statefulUri);
      assertNotNull("[RuleRuntimeTest] " + "Input stream for " + location + "/"
          + statefulUri + " could not be created.", inStream);

      // parse the ruleset from the XML document
      res = ruleAdministrator.getLocalRuleExecutionSetProvider(null)
          .createRuleExecutionSet(inStream, null);
      assertNotNull("[RuleRuntimeTest] " + "RuleExecutionSet " + statefulUri
          + " could not be created.", res);
      inStream.close();

      // register the RuleExecutionSet
      ruleAdministrator.registerRuleExecutionSet(statefulUri, res, null);

      // create a RuleRuntime
      RuleRuntime ruleRuntime = serviceProvider.getRuleRuntime();
      assertNotNull("[RuleRuntimeTest] " + "RuleRuntime not created.",
          ruleRuntime);
      ...
      assertEquals("[RuleRuntimeTest] " + "Not all registrations found.", 2,
          countRegistrations);

      RuleSession ruleSession = null;

      // Create and check a stateless rule session.
      ruleSession = ruleRuntime.createRuleSession(statelessUri, null,
          RuleRuntime.STATELESS_SESSION_TYPE);
      assertNotNull("[RuleRuntimeTest] "
          + "Stateless rule session could not be created.", ruleSession);
      ...
      assertNotNull("[RuleRuntimeTest] "
          + "Stateful rule session could not be created.", ruleSession);
      assertTrue(
          "[RuleRuntimeTest] " + "Rule session is not of Stateful type.",
