 ...
 * <p>
 * This class implements several convenience methods for accessing a rule
 * session. It will parse the "tck.conf" configuration file. The "tck.conf"
 * configuration file is located in the lib directory of the tck distribution.
 * 
 * @version 1.0
 * @since JSR-94 1.0
 */
public abstract class TestCaseUtil {
  ...
  /**
   * Get the rule service provider configuration setting. This method will parse
   * the tck.conf configuration file and return the definition of the
   * rule-service-provider. This definition should contain a valid class name
   * for the rule engine vendor specific implementation of the
   * RuleServiceProvider.
   * 
  ...
  /**
   * This method complements the previous method. We suppose that the rule
   * provider implementation classes are stored in stand-alone JAR files
   * provided as URLs. The method creates a URLClassLoader, loads the
   * RuleServiceProvider class from the class loader, then registers the object
   * using the class loader.
   * 
   * Note: we need to take care about the CLASSPATH when launching this test.
   * 
   * @param uri
   *          The registration name of this rule service provider.
   * @param urls
   *          The URLs which provide the rule service provoider classes.
   * 
   * @return The registered provider.
   * 
   */
  public static RuleServiceProvider getRuleServiceProvider(String uri,
      URL[] urls) throws ClassNotFoundException, ConfigurationException {
    if (ruleServiceProvider == null)
      parseTckConfiguration();
    ...
    TestCase.assertNotNull("[TestCaseUtil] Class loading on "
        + ruleServiceProvider + " failed. ", ruleServiceProviderClass);
  ...
  /**
   * Get the rule execution set location sconfiguration setting. This method
   * will parse the tck.conf configuration file and return the location of the
   * rule execution sets.
   * 
   ...
   * 
   * @return A vector of URLs of the jar file names of the rule service
   *         provider.
   */
   ...
   * <p>
   * Get a rule engine vendor specific rule service provider. The provider is
   * constucted in the following manner:<br>
   * <ul>
   ...
   * <p>
   * Get a stateful rule session. The provider is constucted in the following
   * manner:<br>
   * <ul>
   * <li>Get a RuleServiceProvider {@link #getRuleServiceProvider}.
   * <li>Get the RuleAdministrator and create/register a rule execution set.
   * <li>Get the RuleRuntime and create a rule session on the registered rule
   * execution set.
   * </ul>
   * <p>
   * <b>Note:</b><br>
   * The location of the rule execution sets is specified in the tck.conf
   * configuration file with the tag "rule-execution-set-location".
   * 
   * @param uri
   *          The registration name of this rule service provider.
   * @param ruleExecutionSetUri
   *          The file name of the rule execution set
   * 
   * @return The stateful rule session.
   * 
   */
  public static StatefulRuleSession getStatefulRuleSession(String uri,
      String ruleExecutionSetUri) throws Exception {
    // Get the RuleServiceProvider
    RuleServiceProvider serviceProvider = getRuleServiceProvider(uri);
    TestCase.assertNotNull("[TestCaseUtil] RuleServiceProvider " + uri
        + " not found.", serviceProvider);

    // Get the RuleAdministrator
    RuleAdministrator ruleAdministration = serviceProvider
    ...
    TestCase.assertNotNull("[TestCaseUtil] inStream non null", inStream);

    // parse the ruleset from the XML document
    RuleExecutionSet res = ruleAdministration.getLocalRuleExecutionSetProvider(
        null).createRuleExecutionSet(inStream, null);

    TestCase.assertNotNull("[TestCaseUtil] RuleExecutionSet "
        + ruleExecutionSetUri + " could not be created.", res);
    ...
    StatefulRuleSession session = (StatefulRuleSession) ruleRuntime
        .createRuleSession(uri, null, RuleRuntime.STATEFUL_SESSION_TYPE);
   ...
   * <p>
   * Get a stateless rule session. The provider is constucted in the following
   * manner:<br>
   * <ul>
   * <li>Get a RuleServiceProvider {@link #getRuleServiceProvider}.
   * <li>Get the RuleAdministrator and create/register a rule execution set.
...

  {
    ...
    return (StatelessRuleSession) ruleRuntime.createRuleSession(uri, null,
        RuleRuntime.STATELESS_SESSION_TYPE);

  }

  /**
   * Get a rule execution set input stream. This method will return an input
   * stream to the RuleExecutionSet for the specified uri. <b>Note:</b><br>
   * The location of the rule execution sets is specified in the tck.conf
   * configuration file with the tag "rule-execution-set-location".
   * 
   * @param ruleExecutionSetUri
  ...
  public static InputStream getRuleExecutionSetInputStream(
    ...
    try {
      ...
      TestCase.assertNotNull("[TestCaseUtil] Input stream for " + location
          + "/" + ruleExecutionSetUri + " could not be created.", inStream);
  ...
  /**
   * Get a rule execution set reader. This method will return a Reader to the
   * RuleExecutionSet for the specified uri. <b>Note:</b><br>
   * The location of the rule execution sets is specified in the tck.conf
   * configuration file with the tag "rule-execution-set-location".
   * 
   * @param ruleExecutionSetUri
  ...
  public static Reader getRuleExecutionSetReader(String ruleExecutionSetUri) {
    ...
    try {
      ...
      TestCase.assertNotNull("[TestCaseUtil] Reader for "
          + ruleExecutionSetLocation + "/" + ruleExecutionSetUri
          + " could not be created.", reader);
    } catch (FileNotFoundException e) {
      TestCase.fail(e.getMessage());
    }
    return reader;
  ...
  /**
   * Get the DOM represenation for a rule execution set. This method will return
   * a DOM representing the RuleExecutionSet for the specified uri. <b>Note:</b><br>
   * The location of the rule execution sets is specified in the tck.conf
   * configuration file with the tag "rule-execution-set-location".
   * 
   * @param ruleExecutionSetUri
  ...
  /**
   * Parse the TCK configuration file. This file should contain settings for the
   * following: ruleServiceProvider: The rule engine vendor specific rule
   * service provider implementation. ruleExecutionSetLocation: The location
   * where the TCK test rule execution sets can be found.
   */
  ...
  /**
   * Processes an unexpected exception thrown in a JUnit test method. This
   * message extracts (if possible) the line number from the exception and
   * displays appropriate information for easy debugging. Then it fails the test
   * by calling <code>fail</code>.
   * 
   * @param e
   *          The target exception to be processed.
   * @param test
   *          The test case.
   * @param methName
   *          The method name in which the exception is caught.
   */
  public static void processTestException(Exception e, TestCase test,
      String methName) {
    ...
    if (lineno != null)
      Assert
          .fail("Exception thrown at line " + lineno + ":\n" + e.getMessage());
    else
