...
/**
 * Tests the {@link javax.rules.RuleServiceProviderManager} and
 * {@link javax.rules.RuleServiceProvider} classes. This test complements the
 * others by creating a class loader to test the methods that have a ClassLoader
 * in their signature.
 * 
 * @version 1.0
 * @since JSR-94 1.0
...
public class ClassLoaderTest extends TestCase {
  /**
   * RuleServiceProviderManagerTest constructor. This is the default constructor
  ...
  public void testRuleServiceProviderManager() {
    ...
    assertNotNull("[ClassLoaderTest] "
        + "No jar file name for the rule service provider.", vect);
    assertTrue("[ClassLoaderTest] "
        + "No jar file name for the rule service provider.", vect.size() > 0);
    ...
    try {
      ...
      RuleServiceProvider provider = TestCaseUtil.getRuleServiceProvider(uri,
          urls);

      // Retrieve the provider
      Object object = RuleServiceProviderManager.getRuleServiceProvider(uri);
      assertEquals("[RuleServiceProviderManagerTest] ", provider.getClass(),
          object.getClass());
      ...
      try {
        ...
        assertNull("[RuleServiceProviderManagerTest] "
            + "Deregistered provider still in list of registrations.", object);
