 ...
 * </p>
 * <p>
 * Description: This class is a representation of a stateful rules engine
 * session. A stateful rules engine session exposes a stateful rule execution
 * API to an underlying rules engine. The session allows arbitrary objects to be
 * added and removed to and from the rule session state. Additionally, objects
 * currently part of the rule session state may be updated.
 * 
 ...
 * therefore dependent on the rules within the RuleExecutionSet as well as the
 * rule engine vendor's specific rule engine behavior.
 * 
 * Handle instances are used by the rule engine vendor to track Objects added to
 * the rule session state. This allows multiple instances of equivalent Objects
...
public class StatefulRuleSessionImpl implements StatefulRuleSession {
  ...
  /**
   * @return <code>true</code> if the given object is contained within rule
   *         session state of this rule session, <code>false</code> otherwise
   */
   ...
   */
  public final Handle addObject(Object object)
      throws InvalidRuleSessionException {
    try {
      validateRuleSession();

      if (object instanceof Clause)
        workingMemory.put(((Clause) object).getName(),
            ((Clause) object).getValue());
      else if (object instanceof Component
          && ((Component) object).getName() != null)
  ...
  public final List addObjects(List objList) throws InvalidRuleSessionException {
    ...
    while (it.hasNext())
      al.add(addObject(it.next()));
    return al;
  }

  /**
   * Notifies the rules engine that a given object in the rule session state has
   * changed.
   */
   ...
   */
  public final List getObjects(ObjectFilter filter)
      throws InvalidRuleSessionException {
    validateRuleSession();
    ArrayList al = new ArrayList();
   ...
   * present in the rule session state.
   */
   ...
   */
  public final Object getObject(Handle handle) throws InvalidHandleException,
   ...
   * @param obj
   *          instance containing the method to invoke
   */
  private void executeMethod(Action action, Object obj)
      throws InvalidRuleSessionException {
    Method[] methods = null;
    ...
    try {
      ...
      if (method.indexOf(".") == -1)
        throw new InvalidRuleSessionException("No method '"
            + action.getMethod() + "' found");
      else
      ...
      for (i = 0; i < methods.length; i++)
        if (methods[i].getName().equals(method)) {
          if (action.getValues().size() == methods[i].getParameterTypes().length) {
            // method found...
            List values = action.getValues(); // values to set (in String
                                              // format)
            ...
            for (int j = 0; j < values.size(); j++) {
              ...
              if (value.indexOf(".") != -1) {
                ...
                Object aux = workingMemory.get(value.substring(0,
                    value.lastIndexOf(".")));
                try {
                  if (aux != null)
                    value = aux
                        .getClass()
                        .getMethod(value.substring(value.lastIndexOf(".") + 1),
                            new Class[0]).invoke(aux, new Object[0]).toString();
                } catch (NullPointerException ex) {
                  value = null;
                } catch (Throwable ex) {
                  throw new InvalidRuleSessionException("Method not found: '"
              ...
              if (methods[i].getParameterTypes()[j] == double.class)
                ...
                argValue = methods[i].getParameterTypes()[j].getConstructor(
            ...
            if (obj instanceof Clause)
              workingMemory.put(((Clause) obj).getName(),
                  ((Clause) obj).getValue());
            else if (obj instanceof Component
                && ((Component) obj).getName() != null)
      ...
      throw new InvalidRuleSessionException("No method '" + action.getMethod()
          + "' found in class '" + obj.getClass().getName() + "'");
    } catch (Throwable ex) {
      String msg = "";
      if (obj != null)
  ...
  /**
   * Analyze all rules and fire those rules that are valid. After rule firing
   * all rules are re-analyzed.
   * 
   * @param rulesAlreadyFired
   *          rules already fired
   */
  private void forwardChaining(HashSet rulesAlreadyFired)
      throws InvalidRuleSessionException {
    RuleImpl rule = null;
    Assumption ass = null;
    Action action = null;
    ...
    for (int i = 0; i < ruleSet.getRules().size(); i++) {
      ...
      for (int j = 0; j < rule.getAssumptions().size(); j++) {
        ...
        if (t1.indexOf(".") != -1) {
          ...
          try {
            if (aux != null) {
              ...
              Method auxMethod = auxClass.getMethod(
                  t1.substring(t1.lastIndexOf(".") + 1), new Class[0]);
          ...
          // try {
          // if (aux != null)
          // t1 = aux.getClass().getMethod(t1.substring(t1.lastIndexOf(".")+1),
          // new Class[0]).invoke(aux, new Object[0]).toString();
          // }
          catch (NullPointerException ex) {
            t1 = null;
        ...
        if (t2.indexOf(".") != -1) {
          ...
          try {
            if (aux != null) {
              Class auxClass = aux.getClass();
              Method auxMethod = auxClass.getMethod(
                  t2.substring(t2.lastIndexOf(".") + 1), new Class[0]);
              Object auxResult = auxMethod.invoke(aux, new Object[0]);
          ...
          // try {
          // if (aux != null)
          // t2 = aux.getClass().getMethod(t2.substring(t2.lastIndexOf(".")+1),
          // new Class[0]).invoke(aux, new Object[0]).toString();
          // }
          catch (NullPointerException ex) {
            t2 = null;
            ...
            throw new InvalidRuleSessionException("Method not found: '"
                + t2.substring(0, t2.lastIndexOf(".") + 1) + "'");
          }
        ...
        if (ass.getOperator().equals(Assumption.EXISTS)) {
          ...
          if (t1 != null && t1.startsWith(":") && t2 != null) { // t1 is a
                                                                // variable...
            Enumeration en = workingMemory.keys();
            ...
            ok = t1 == null && t2 == null || t1 != null && t2 != null
                && t1.equals(t2);
        } // PART 0: 11.04.2008 - MAURIZIO FABBRI - Added management of
          // Assumption.CONTAINSATLEASTONE
        else if (ass.getOperator().equals(Assumption.CONTAINSATLEASTONE)) {
          if (t1 != null && t2 != null && t2.startsWith("[")
              && t2.endsWith("]")) {
            ...
            for (int t2i = 0; t2i < t2List.length && !stop; t2i++) {
              // System.out.println(t1 + " contiene " + t2List[t2i] + " = " +
              // t1.indexOf(t2List[t2i]));
            ...
            ok = t1 != null && t2 != null && t1.indexOf(t2) != -1;
          }
        } // End PART 0
          ...
          if (t1 != null && t1.startsWith(":") && t2 != null) { // t1 is a
            ...
            // PART 1: 11.04.2008 - MAURIZIO FABBRI - Added management of the
            // second term of type HashSet
            if (t1 != null && t2 != null && t2.startsWith("[")
                && t2.endsWith("]")) {
              t2 = t2.substring(1, t2.length() - 1);
              String[] t2List = t2.split(",");
          ...
          // PART 2: 11.04.2008 - MAURIZIO FABBRI - Added management of the
          // second term of type HashSet
          if (t1 != null && t2 != null && t2.startsWith("[")
              && t2.endsWith("]")) {
            t2 = t2.substring(1, t2.length() - 1);
          ...
          // PART 3: 11.04.2008 - MAURIZIO FABBRI - Added management of
          // Assumption.NOT_CONTAINSANYONE
          if (t1 != null && t2 != null && t2.startsWith("[")
              && t2.endsWith("]")) {
            t2 = t2.substring(1, t2.length() - 1);
            String[] t2List = t2.split(",");
          ...
          try {
            ok = t1 != null
                ...
                && t2 != null
                && !"".equals(t2)
                && (new Double(t1)).doubleValue() < (new Double(t2))
                    .doubleValue();
          } catch (NumberFormatException ex1) {
            throw new InvalidRuleSessionException("Numeric type expected: '"
                + t1 + "','" + t2 + "'");
          }
          ...
          throw new InvalidRuleSessionException("Operator not supported: '"
              + ass.getOperator() + "'");
        ...
        for (int k = 0; k < action.getValues().size(); k++)
          if (action.getValues().get(k).toString().startsWith(":")) {
            ...
            if (variables.containsKey(action.getValues().get(k).toString()))
              action.getValues().set(k,
                  variables.get(action.getValues().get(k)));
            else
              throw new InvalidRuleSessionException("Variable '"
                  + action.getValues().get(k) + "' not bound");
          }

        if (action.getMethod().indexOf(".") != -1) {
          className = action.getMethod().substring(0,
              action.getMethod().lastIndexOf("."));
          ...
          try {
            ...
            throw new InvalidRuleSessionException("Class not found '"
                + className + "'");
          }
        } else
          throw new InvalidRuleSessionException("Class not found '"
  ...
  /**
   * @return method not included into JSR specifications: used to access to
   *         working memory
   */
