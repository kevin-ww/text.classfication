 ...
 * </p>
 * <p>
 * Description: Local Rule Execution Set Provider Implementation.
 * </p>
 * <p>
...
public class LocalRuleExecutionSetProviderImpl implements
  ...
  /**
   * Creates a RuleExecutionSet implementation using a supplied input stream and
   * additional vendor-specific properties.
   */
  public RuleExecutionSet createRuleExecutionSet(InputStream inputStream,
      Map props) throws RuleExecutionSetCreateException, IOException {
    Document doc = null;
    try {
  ...
  /**
   * Creates a RuleExecutionSet implementation using a supplied character stream
   * Reader and additional vendor-specific properties.
   */
  ...
  /**
   * Creates a RuleExecutionSet implementation from a vendor specific AST
   * representation and vendor-specific properties.
   */
  public RuleExecutionSet createRuleExecutionSet(Object ast, Map properties)
    ...
    if (ast == null || !(ast instanceof Collection))
      throw new RuleExecutionSetCreateException(
          "Invalid type argument: AST argument must be a collection of RuleImpl objects");
    if (properties == null) {
   ...
   */
  private RuleExecutionSet createRuleExecutionSet(Element docElement,
    ...
    try {
      ...
      String description = contents.item(0).getFirstChild().getNodeValue()
          .trim();
      ...
      for (int i = 0; i < contents.getLength(); i++) {
        ...
        for (int j = 0; j < nodes.getLength(); j++) {
          ...
          if (node.getAttributeNode("op") != null
            ...
            assumptions.add(new Assumption(parseClassName(
                node.getAttribute("leftTerm"), synonymns), node
                .getAttribute("op"), parseClassName(
                node.getAttribute("rightTerm"), synonymns)));
          else
            assumptions.add(new Assumption(parseClassName(
                node.getAttribute("leftTerm"), synonymns)));
        }
        ...
        for (int j = 0; j < nodes.getLength(); j++) {
          ...
          actions.add(new Action(parseClassName(node.getAttribute("method"),
              synonymns), args));
        }
        rules.add(new RuleImpl(el.getAttribute("name"), el
            .getAttribute("description"), assumptions, actions));
      }
   ...
   */
  private RuleExecutionSet createRuleExecutionSetFromRuleList(Collection rules,
      Map props) throws RuleExecutionSetCreateException, IOException {
    try {
